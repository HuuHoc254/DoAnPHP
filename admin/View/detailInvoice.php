...
<div id="layoutSidenav_content">
    <main>
      ...
      <h5 class="ml-3   ">Số tiền đã thanh toán: <?php echo number_format($total+$invoice->getShipCode(),0,'.',' ')." VND" ?></h5>
      <?php elseif($invoice->getStatus()=="Đã giao hàng"): ?>
        <h5 class="ml-3   ">Số tiền đã thanh toán: <?php echo number_format($total+$invoice->getShipCode(),0,'.',' ')." VND" ?></h5>
        <h5 class="ml-3   ">Trạng thái : Đang giao hàng</h5>
        <?php else:?>
          <h5 class="ml-3   ">Trạng thái : Đã hủy đơn.</h5> 
          <?php endif; ?>             
                </div>
            </div>
        </div>
