...
session_start();
  ...
  if($action=="login"){
    ...
    $productDB->updateView($product_id,$view+1);
      ...
      header('Location: product_list.php?search='.$search);
        ...
        if($quantity>$p->getQuantity()){
          $message = "<h2 class='text-danger'>Số lượng sản phẩm trong kho không đủ!</h2>";
          $_SESSION['mess']=$message;
    ...
    $new_qty_list = filter_input(INPUT_POST, 'newqty', 
        ...
        foreach($new_qty_list as $key => $qty) {
          ...
          if($qty>$p->getQuantity()){
            $message = "<h2 class='text-danger'>".$p->getProduct_name()." trong kho không đủ !</h2>
                        Trong kho chỉ còn lại ".$p->getQuantity()." sản phẩm";
            $_SESSION['mess']=$message;
            header('Location: ../View/cart.php');
    ...
    $_SESSION['cart13'] = $cart;
    $message = "<h2 class='text-danger'>Bạn đã update đơn hàng!</h2>";
    $_SESSION['mess']=$message;
    header('Location: ../View/cart.php');}
    ...
    $_SESSION['cart13'] = $cart;
    $message = "<h2 class='text-danger'>Bạn đã xóa đơn hàng!</h2>";
    $_SESSION['mess']=$message;
    header('Location: ../View/cart.php');
    ...
    $message = "<h1 class='text-danger'>Mật khẩu không chính xác!</h1>";
      ...
      if($pass!=$accountDB->getAccount_by_userName($_SESSION["login"])->getPassword()){
        $message = "<p class='text-danger'>Mật khẩu không chính xác!</p>";
       $_SESSION['mess']=$message;
      ...
      if($newPass!=$reNewPass){
        $message = "<h1 class='text-danger'>Mật khẩu không trùng khớp!</h1>";
        $_SESSION['mess']=$message;
        header('Location: ../View/update_password.php');
      }else {
  ...
  if($pass!=$accountDB->getAccount_by_userName($_SESSION["login"])->getPassword()){
    ...
    if ($amount<murach\cart\get_subtotal($cart)){
      $message = "<h2 class='text-danger'>Số tiền trong tài khoản không đủ để thực hiện giao dịch!</h2>
                  <p>Vui lòng nạp thêm tiền để tiếp tục thanh toán.</p>";
      $_SESSION['mess']=$message;
      header('Location: ../View/pay.php');
    }else{
      $address = filter_input(INPUT_POST, 'address');
      ...
      $message = "<h1 class='text-danger'>Thanh toán thành công!</h1>
      <p>Bạn đã thanh toán giao dịch thành công. Đơn hàng sẽ sớm được giao tới nhà của bạn.</p>";
      $_SESSION['mess']=$message;
      header('Location: ../View/detailInvoice.php?action=detailInvoice&invoiceId='.$invoiceId);
    }
  }
}else if($action=="loginFalse"){
  $message = "<h2 class='text-danger'>Bạn cần đăng nhập để thanh toán!</h2>";
  $_SESSION['mess']=$message;
  ...
  $message = "<h3 class='text-danger'>Bạn đã hủy đơn hàng thành công!</h3>";
  $_SESSION['mess']=$message;
  header('Location: ../View/account.php');
  ...
  $message = "<h3 class='text-danger'>Bạn đã phản hồi thành công!</h3>";
  $_SESSION['mess']=$message;
